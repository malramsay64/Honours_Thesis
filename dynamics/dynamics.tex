...
\begin{equation}
    \tau_1, D \propto \e^{a^2/2\rangle u^2\langle}
\end{equation}
where $a$ is the displacement to overcome the local energy barrier and $\rangle u^2 \langle$ is the Debye-Waller factor. We are primarily concerned with the Debye-Waller factor a measure of the local rigidity of the structure. The Debye-Waller factor is found as the MSD at the time where $d\log{MSD}/d\log{t}$ is a minimised, where the slope is lowest in \textfigref{msd}.
