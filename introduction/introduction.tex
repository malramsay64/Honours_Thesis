...
The liquid phase exists due to an attractive force that extends far enough~\figref{fig:potentials} for a particles to move past one another while still feeling the attractive force~\cite{tejero:94}. When the potential well becomes too narrow the liquid phase ceases to exist leaving only the solid and gas phases.
...
The structure of the liquid is defined by the repulsive forces, with particles forming distinct shells~\figref{fig:radial distribution}. These shells however decay over distance, leaving an isotropic distribution. The \emph{radial distribution function} $g(r)$ is the distribution of particles found at a distance $r$ from any other particle,
\begin{align}
...
The radial distribution function is also crucial in comparing simulation to experiment. By performing a transformation \towrite{specifics} we get the structure factor. The structure factor is another two particle interaction that is experimentally determined from x-ray diffraction patterns.
